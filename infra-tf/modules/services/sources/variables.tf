...
variable "discovery_ocsp_security_group_id" {
  description = "Dedicated OCSP-egress security group ID (from network). Attached as a SECOND SG on the sources-db-connector Lambda so its Snowflake driver can reach port-80 OCSP responders without granting that egress to every other Lambda on the shared lambda SG."
  type        = string
}
